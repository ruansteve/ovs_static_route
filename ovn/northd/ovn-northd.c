...
static void
build_static_route_flow(struct hmap *lflows, struct ovn_datapath *od, struct hmap *ports,
        const struct nbrec_logical_router_static_route *route)
{
    ...
    if (error || mask != OVS_BE32_MAX) {
        ...
        VLOG_WARN_RL(&rl, "bad 'nexthop' %s error %s", route->nexthop, error);
        free(error);
        return;
    }
    ...
    if (error || !ip_is_cidr(mask)) {
        static struct vlog_rate_limit rl
                        = VLOG_RATE_LIMIT_INIT(5, 10);
        VLOG_WARN_RL(&rl, "bad 'prefix' %s error %s", route->prefix, error);
        free(error);
        return;
    }
